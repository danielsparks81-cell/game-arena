...
export default async function LobbyPage() {
  ...
  const { data: rooms } = await supabase
    .from('rooms')
    .select('id, game_type, status, host_id, created_at, room_players(player_id, seat, profiles(username, accent_color))')
    .order('created_at', { ascending: false })
    .limit(30);

  const { data: stats } = await supabase
    .from('user_stats')
    .select('user_id, username, accent_color, wins, losses, draws, games');
